...
function toFeatureCollection(dataPoints: DataPoint[]): GeoJSON.FeatureCollection {
  return {
    ...
    features: dataPoints.map((dp) => ({
      ...
      properties: { id: dp.id, timestamp: dp.timestamp, value: dp.value },
    })),
  };
}
...
function toRouteFeatureCollection(dataPoints: DataPoint[]): GeoJSON.FeatureCollection {
  ...
  const coordinates = sorted
    .map((dp) => dp.geometry.type === 'Point' ? dp.geometry.coordinates : null)
...
function addPointLayer(map: mapboxgl.Map, sourceId: string, layerId: string) {
  map.addLayer({
    ...
    paint: {
      'circle-radius': ['interpolate', ['linear'], ['get', 'value'], 0, 4, 50, 8, 100, 14],
      'circle-color': ['interpolate', ['linear'], ['get', 'value'], 0, '#2dc4b2', 25, '#3bb3c3', 50, '#669ec4', 75, '#8b88b6', 100, '#a2719b'],
      'circle-opacity': 0.8,
    },
  });
}
...
function addHeatmapLayer(map: mapboxgl.Map, sourceId: string, layerId: string) {
  map.addLayer({
    ...
    paint: {
      ...
      'heatmap-intensity': 1, 'heatmap-radius': 20, 'heatmap-opacity': 0.7,
    },
  });
}
...
function addClusterLayers(map: mapboxgl.Map, sourceId: string, layerId: string) {
  map.addLayer({
    id: `${layerId}-clusters`, type: 'circle', source: sourceId,
  ...
  map.addLayer({
    id: `${layerId}-cluster-count`, type: 'symbol', source: sourceId,
    filter: ['has', 'point_count'],
    layout: { 'text-field': ['get', 'point_count_abbreviated'], 'text-size': 12 },
  });
  map.addLayer({
    id: `${layerId}-unclustered`, type: 'circle', source: sourceId,
    filter: ['!', ['has', 'point_count']],
...
function addChoroplethLayer(map: mapboxgl.Map, sourceId: string, layerId: string) {
  map.addLayer({
    ...
    paint: {
      'fill-color': ['interpolate', ['linear'], ['get', 'value'], 0, '#f7fcf5', 25, '#c7e9c0', 50, '#74c476', 75, '#238b45', 100, '#00441b'],
      'fill-opacity': 0.6,
    },
  });
}
...
function removeLayers(map: mapboxgl.Map, layerId: string) {
  for (const id of [layerId, `${layerId}-clusters`, `${layerId}-cluster-count`, `${layerId}-unclustered`]) {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      if (renderType === 'cluster') {
        map.addSource(sourceId, { type: 'geojson', data: toFeatureCollection(dataPoints), cluster: true, clusterMaxZoom: 14, clusterRadius: 50 });
        addClusterLayers(map, sourceId, layerId);
      } else if (renderType === 'route') {
        map.addSource(sourceId, { type: 'geojson', data: toRouteFeatureCollection(dataPoints) });
        addRouteLayer(map, sourceId, layerId);
      } else {
        map.addSource(sourceId, { type: 'geojson', data: toFeatureCollection(dataPoints) });
        switch (renderType) {
          case 'point': addPointLayer(map, sourceId, layerId); break;
          case 'heatmap': addHeatmapLayer(map, sourceId, layerId); break;
          case 'choropleth': addChoroplethLayer(map, sourceId, layerId); break;
        }
      }
